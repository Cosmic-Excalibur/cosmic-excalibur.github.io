data = lambda: {
    '*.html': {
        __TITLE__: r"""About me""",
        __HTML_TITLE__: r"""~ About me ~""",
        __CONTENT__: r"""
	<div class="main">
		<p>Welcome to <span class="astrageldon">Astrageldon</span>'s blog site😜</p>
        <p>It is strongly recommended to visit this site on a PC rather than a mobile phone.</p>
		<br>
		
		<ul class="section"><li>
			What is <span class="astrageldon">Astrageldon</span> 🤔?
		</li></ul>
		<p><i>Disambiguation: <span class="astrageldon">Astrageldon</span> is not the <span underlined>Astrageldon Slime</span> which directs you <a href="https://calamitymod.wiki.gg/wiki/Astrageldon_Slime" underlined>here</a></i>
		<p><span class="astrageldon">Astrageldon</span> is the owner of this site and will be 332 years old on January 14, 2338, which is 9894009600 seconds after the time this page is created. <span class="astrageldon">Astrageldon</span> is currently a <a href="https://en.wikipedia.org/wiki/Capture_the_flag_(cybersecurity)" underlined>CTFer</a>, but actually more interested in various branches of mathematics as well as their intersection—the Cryptography🔐. Most of the time, <span class="astrageldon">Astrageldon</span> is a technological Otaku. <span class="astrageldon">Astrageldon</span> might be intrigued by stuff like below: (unordered)</p>
		<ul><details>
			<summary></summary>
			<li>Most categories of mathematics</li>
			<li>Programming</li>
			<li>Cybersecurity / Infosec</li>
			<li>Geeky stuff</li>
			<li>Multilingualism</li>
			<li>Algorithms</li>
			<li>Artificial Intelligence🤖</li>
			<li>Memes</li>
			<li>Pixel games <comment>(like Minecraft, Terraria, Undertale, Omori...)</comment></li>
			<li>Electronic Dance Music🎶 <comment>(separate out some subgenres (or not) :p)</comment><ul><details>
				<summary></summary>
				<li>Mainstream Hardcore</li>
				<li>Euphoric Hardstyle</li>
				<li>Rawstyle</li>
				<li>Rawphoric Hardstyle</li>
                <li>Xtra Raw</li>
                <li>Frenchcore</li>
				<li>Psystyle</li>
				<li>Terrorcore</li>
				<li>Uptempo Hardcore</li>
				<li no-marker>... <comment>(so much :o)</li>
				<li>Speedcore</li>
				<li>UK Hardcore</li>
				<li>Psychedelic Trance</li>
				<li>Uplifting Trance</li>
				<li>Techno</li>
				<li>Gabber</li>
				<li>Deathcore</li>
				<li>Color Bass</li>
				<li>Breakcore / Jungle</li>
				<li>Dubstep</li>
				<li>J-core</li>
				<li>Hardbass</li>
				<li>Melodic House</li>
				<li>Tropical House</li>
				<li>Electro House</li>
				<li>Bass House</li>
				<li>Melbourne Bounce</li>
				<li>Complextro</li>
				<li>Moombahcore</li>
				<li>Future Bass</li>
				<li>Kawaii Bass</li>
				<li no-marker>... <comment>(so much :o)</li>
			</details></ul></li>
			<li>Supernaturality</li>
			<li>Cthulhu</li>
			<li>Physics</li>
            <li>Psychology</li>
            <li>Fundamentals of a complicated system</li>
            <li>Politics</li>
			<li no-marker>... <comment>(thinking...)</comment></li>
		</details></ul>
		<br>
		
		<ul class="section"><li>
			What is this site for 🤔?
		</li></ul>
		<p>This silly site is mainly intended for the collection of the owner's brilliant or stupid ideas and for the convenience of looking them up. Mostly, scientific and technological contents are covered. <span class="astrageldon">Astrageldon</span> would like to deploy this site on <a href="https://github.com/Cosmic-Excalibur" underlined>GitHub</a> and as a result it is static :p</p>
		<br>
	</div>
        """
    }
}